...
const InfoCard: React.FC<InfoCardProps> = ({ imageUrl, title, subtitle, address, phone, email, funFacts }) => {
  return (
    <Box className="info-window">
      <Image src={imageUrl} alt={title} />
      <Box className="info-window-content" p={4}>
        <Heading as="h2" size="md">{title}</Heading>
        <Heading as="h4" size="sm" mt={2}>{subtitle}</Heading>
        ...
        {funFacts && <Text><strong>Fun Facts:</strong> {funFacts}</Text>}
      </Box>
    </Box>
  );
};
